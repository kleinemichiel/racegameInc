...
void MI0283QT9::touchStartCal(void)
{
 ...
 // drawTextPGM((lcd_width/2)-50, (lcd_height/2)-10, PSTR("Calibration"), RGB(0,0,0), RGB(255,255,255), 1);
  ...
  {
    //draw points
    drawRect(lcd_points[i].x, lcd_points[i].y,  2, 2, RGB(  0,  0,  0));
    drawRect(lcd_points[i].x, lcd_points[i].y,  5, 5,  RGB(  0,  0,  0));
    drawRect(lcd_points[i].x, lcd_points[i].y, 10, 10, RGB(255,  0,  0));

    //press dectected? -> save point
    if(touchRead())
    {
      fillRect(lcd_points[i].x, lcd_points[i].y, 2, 2, RGB(255,0,0));
      tp_points[i].x = tp_x;
      tp_points[i].y = tp_y;
      i++;
