...
extern void drawCar (MI0283QT9 lcd, uint8_t x, uint8_t y, uint8_t rgb[2], uint8_t isPlayer){
	...
	if(isPlayer){
		lcd.drawInteger(pixelPosX(x) + 18, pixelPosY(y) + 29, isPlayer, DEC, RGB(rgb[0],rgb[1],rgb[2]), BACKGROUND, 1);
	}
...
extern void removeCar(MI0283QT9 lcd, uint8_t x, uint8_t y, uint8_t isPlayer){
	...
	if(isPlayer){
		lcd.drawInteger(pixelPosX(x) + 18, pixelPosY(y) + 29, isPlayer, DEC, BACKGROUND, BACKGROUND, 1);
	}
