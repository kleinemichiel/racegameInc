...
#include "GameLoop.h"
 ...
 //this array will contain the objects with their data
 objects object[2];
 ...
 void showGenObjects(MI0283QT9 lcd){
	 ...
	 */
	 ...
	 if(object[0].type == 1){
		 removeCar(lcd, prevposXobj1, object[0].posY, 0);
		 drawCar(lcd, object[0].posX, posYobj1, object[0].rgb, 0);
	 ...
	 if(posYobj1 > 6 || keepObj2Alive){
		 ...
		 keepObj2Alive = 1;
		  ...
		  if(object[1].type == 1){
			  if(posYobj2 < 128){
				  removeCar(lcd, prevposXobj2, object[1].posY, 0);
				  drawCar(lcd, object[1].posX, posYobj2, object[1].rgb, 0);
	 ...
	 if(posYobj1 > 12 || keepObj3Alive){
		 ...
		 if(object[2].type == 1){
			 if(posYobj3 < 128){
				 removeCar(lcd, prevposXobj3, object[2].posY, 0);
				 drawCar(lcd, object[2].posX, posYobj3, object[2].rgb, 0);
...
extern void showGame(MI0283QT9 lcd){
	...
	//set sensitivity for tilt sensor
	...
	//start nunchuck communication
	nun.nunchuck_init();
	
	//for insurance set every object position at 0
	posYobj1 = 0;
	posYobj2 = 0;
	posYobj3 = 0;
	...
	while(1){
		...
		//if brake back into this loop, reset everything if restartGame is set
		if(restartGame){
		...
		drawCar(lcd, x, 12, colored, 1);
		
		//if multiplayer is enabled, draw other player
		if(isConnected){
			removeCar(lcd, prevClientX, 12, 2);
			uint8_t clientPosition = getClientPosition();
			
			while(!clientPosition)
			drawCar(lcd, clientPosition, 12, colored, 2);
			
			//check if other player died
			if(getGameOver()){
				showGameOverMenuMP(lcd, score, 1);
			}
			prevClientX = clientPosition;
		}
		...
		if((object[0].type == 1 && object[0].posY >= 12  && object[0].posY <= carHitMax ) || (object[0].type == 2 && object[0].posY >= 12 && object[0].posY <= truckHitmax)){
			if (object[0].posX == x ) {
				//if multiplayer mode, go to custom gameover menu after sending game over to other player
				if(isConnected){
					sendGameOver();
					_delay_ms(100);
					showGameOverMenuMP(lcd, score, 0);
				} else {
		...
		if(nun.retreive_data()){
			...
			if(nun.getAccX() > 148 || nun.getJoyX() > 150){
				...
				//draw car at x location and send it over to other player if in multiplayer mode
				sendHostPosition(x);
				drawCar(lcd, x, 12, colored, 1);
